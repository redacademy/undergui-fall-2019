...
function utg_entry_footer()
{
	...
	if ('post' === get_post_type()) {
		...
		if ($categories_list && utg_categorized_blog()) {
 ...
 * @return bool
 */
function utg_categorized_blog()
{
	if (false === ($all_the_cool_cats = get_transient('utg_categories'))) {
		...
		set_transient('utg_categories', $all_the_cool_cats);
	}

	if ($all_the_cool_cats > 1) {
		// This blog has more than 1 category so utg_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so utg_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in utg_categorized_blog.
 */
function utg_category_transient_flusher()
{
	if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) {
		return;
	}
	// Like, beat it. Dig?
	delete_transient('utg_categories');
}
add_action('edit_category', 'utg_category_transient_flusher');
add_action('save_post',     'utg_category_transient_flusher');
 ...
 */
function utg_comment_list($comment, $args, $depth)
{
	 ...
	 */
	function utg_numbered_pagination()
	{
		global $wp_query;
		$big = 999999999;
